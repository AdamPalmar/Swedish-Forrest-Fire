...
data = pd.read_csv('C:/Users/Thor/Desktop/5.semester/Swedish-Forrest-Fire/CleanSet.csv')
#print mat_data
#X = np.matrix(mat_data['X'])
...
X = np.matrix(data_normalized.drop(['reportedCrime','reportedCrimeVandalism','Cases', 'medianIncome'],axis=1).dropna())
# y = np.matrix((data['Cases'] / data['population']) *1e3)
y = np.matrix(data['reportedCrime']).T

attributeNames = data_normalized.drop(['reportedCrime','reportedCrimeVandalism','Cases', 'medianIncome'],axis=1).dropna().columns.values
N, M = X.shape
